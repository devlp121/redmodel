...
for path, dataset in [(train_path, unsup_set)]:
    with tf.io.TFRecordWriter(path) as unsup_writer:
        for example in dataset:
            feature = example[1]
            unsup_writer.write(
...
def _create_pipeline(pipeline_name: Text, pipeline_root: Text, data_root: Text,
    ...
    return pipeline.Pipeline(
        ...
        components=[
            example_gen, identify_examples, statistics_gen, schema_gen, validate_stats, synthesize_graph, transform, graph_augmentation, trainer, model_validator, pusher
        ],
        enable_cache=True,
